...
#define QC_CONVENTIONS "U.S. Integrated Ocean Observing System, 2017. Manual for the Use of Real-Time Oceanographic Data Quality Control Flags, Version 1.1"
...
NemoFeedbackWriter::NemoFeedbackWriter(eckit::PathName& filename, const std::vector<double> & lons,
       const std::vector<double> & lats, const std::vector<double> & depths,
       const std::vector<double> & times, const std::vector<std::string> & variable_names,
  ...
  oops::Log::debug() << "nemo_feedback::NemoFieldReader::NemoFieldReader filename : "
  ...
  if(ncFile->isNull()) {
    ...
    err_stream << "nemo_feedback::NemoFieldReader::NemoFieldReader cannot open " << filename
               << std::endl;
  ...
  write_metadata_variables(variable_names,  additional_variables, juld_reference);
...
void NemoFeedbackWriter::define_coord_variables(const size_t n_obs, const size_t n_levels, 
  ...
  nlevels_dim = std::make_unique<netCDF::NcDim>(ncFile->addDim(N_LEVELS, n_levels));
  tmp_Dim = ncFile->addDim(N_VARS, n_obs_vars);
  tmp_Dim = ncFile->addDim(N_ENTRIES, n_add_entries);

}

void NemoFeedbackWriter::write_metadata_variables(const std::vector<std::string>& variable_names, 
    const std::vector<std::string> & additional_variables, const util::DateTime& juld_reference) {
...

  {
    ...
    std::vector<netCDF::NcDim> dims{ncFile->getDim(N_VARS), ncFile->getDim(STRINGNAM)};
    netCDF::NcVar nc_var_list_var = ncFile->addVar("VARIABLES", netCDF::ncChar, dims);
  ...
  {
    size_t max_n_add_entries = additional_variables.size();
    std::vector<netCDF::NcDim> dims{ncFile->getDim(N_ENTRIES), ncFile->getDim(STRINGNAM)};
    netCDF::NcVar nc_entries_var = ncFile->addVar("ENTRIES", netCDF::ncChar, dims);
    nc_entries_var.putAtt("long_name", "List of additional entries for each variable in feedback files");
    fixed_length_name_type *data;
...
/// define metadata variables for each report, applying to all observations in that report. e.g:
/// STATION_IDENTIFIER, STATION_TYPE, DEPTH_QC, DEPTH_QC_FLAGS,
/// OBSERVATION_QC, OBSERVATION_QC_FLAGS, JULD_QC, JULD_QC_FLAGS, POSITION_QC,
/// POSITION_QC_FLAGS
void NemoFeedbackWriter::define_whole_report_variables(const size_t n_obs, const size_t n_levels) {
  
  {
    const std::vector<netCDF::NcDim> dims{*nobs_dim, ncFile->getDim(STRINGWMO)};
    netCDF::NcVar tmp_var = ncFile->addVar("STATION_IDENTIFIER", netCDF::ncInt, dims);
  ...
  {
    const std::vector<netCDF::NcDim> dims{*nobs_dim, *nlevels_dim, ncFile->getDim(N_QCF)};
    netCDF::NcVar tmp_var = ncFile->addVar("DEPTH_QC_FLAGS", netCDF::ncInt, dims);
    tmp_var.putAtt("long_name", "Quality on depth");
    tmp_var.putAtt("Conventions", "OPS flag conventions");
  }

  {
    netCDF::NcVar tmp_var = ncFile->addVar("OBSERVATION_QC", netCDF::ncInt, *nobs_dim);
...

  {
    ...
    netCDF::NcVar tmp_var = ncFile->addVar("OBSERVATION_QC_FLAGS", netCDF::ncInt, dims);
    tmp_var.putAtt("long_name", "Quality on observation");
    tmp_var.putAtt("Conventions", "OPS flag conventions");
  }

  {
    netCDF::NcVar tmp_var = ncFile->addVar("POSITION_QC", netCDF::ncInt, *nobs_dim);
  ...
  {
    const std::vector<netCDF::NcDim> dims{*nobs_dim, ncFile->getDim(N_QCF)};
    netCDF::NcVar tmp_var = ncFile->addVar("POSITION_QC_FLAGS", netCDF::ncInt, dims);
    tmp_var.putAtt("long_name", "Quality on position");
    tmp_var.putAtt("Conventions", "OPS flag conventions");
  }
  ...
  {
    const std::vector<netCDF::NcDim> dims{*nobs_dim, ncFile->getDim(N_QCF)};
    netCDF::NcVar tmp_var = ncFile->addVar("JULD_QC_FLAGS", netCDF::ncInt, dims);
    tmp_var.putAtt("long_name", "Quality on date and time");
    tmp_var.putAtt("Conventions", "OPS flag conventions");
  }
...
void NemoFeedbackWriter::define_variable(const std::string & variable_name,
  ...
  netCDF::NcVar obs_var = ncFile->addVar(variable_name + "_OBS", netCDF::ncDouble, dims);
    
  for (const auto &name : additional_names) {
    netCDF::NcVar add_var = ncFile->addVar(variable_name + "_" + name, netCDF::ncDouble, dims);
  }
  
  {
    const std::vector<netCDF::NcDim> qcf_dims{*nobs_dim, *nqcf_dim};
    netCDF::NcVar qc_flags_var = ncFile->addVar(variable_name + "_QC_FLAGS", netCDF::ncDouble, qcf_dims);
  }

  {
    const std::vector<netCDF::NcDim> lvl_qcf_dims{*nobs_dim, *nlevels_dim, *nqcf_dim};
    netCDF::NcVar level_qc_flags_var = ncFile->addVar(variable_name + "_LEVEL_QC_FLAGS",
        netCDF::ncInt, lvl_qcf_dims);
  }

  netCDF::NcVar qc_var = ncFile->addVar( variable_name + "_QC", netCDF::ncInt, *nobs_dim);
  qc_var.putAtt("Conventions", QC_CONVENTIONS);

  netCDF::NcVar level_qc_var = ncFile->addVar( variable_name + "_LEVEL_QC", netCDF::ncInt, dims);
  level_qc_var.putAtt("Conventions", QC_CONVENTIONS);

  {
    netCDF::NcVar tmp_var;
    tmp_var = ncFile->addVar(variable_name + "_IOBSI", netCDF::ncInt, *nobs_dim);
    tmp_var.putAtt("long_name", "ORCA grid search I coordinate");
    tmp_var = ncFile->addVar(variable_name + "_IOBSJ", netCDF::ncInt, *nobs_dim);
    ...
    tmp_var = ncFile->addVar(variable_name + "_GRID", netCDF::ncChar, ncFile->getDim(STRINGGRID));
...
void NemoFeedbackWriter::write_coord_variables(const std::vector<double>& lons, const std::vector<double>& lats,
    const std::vector<double>& levels, const std::vector<double>& times) {

  netCDF::NcVar lat_var = ncFile->addVar("LATITUDE", netCDF::ncDouble, *nobs_dim);
  lat_var.putAtt("units", "degrees_north");
  lat_var.putAtt("long_name", "latitude");
  lat_var.putVar(lats.data());

  netCDF::NcVar lon_var = ncFile->addVar("LONGITUDE", netCDF::ncDouble, *nobs_dim);
  lon_var.putAtt("units", "degrees_east");
  lon_var.putAtt("long_name", "longitude");
  lon_var.putVar(lons.data());

  const std::vector<netCDF::NcDim> dims{*nobs_dim, *nlevels_dim};
  netCDF::NcVar depth_var = ncFile->addVar("DEPTH", netCDF::ncDouble, dims);
...
void NemoFeedbackWriter::write_variable_surf(const std::string & variable_name,
  ...
  oops::Log::trace() << "NemoFeedbackWriter::write_variable_surf: writing " << variable_name << std::endl;
  auto surf_var = ncFile->getVar(variable_name);
  surf_var.putVar(data.data());

}

void NemoFeedbackWriter::write_variable_surf_qc(const std::string & variable_name,
    const std::vector<int32_t>& data) {

  oops::Log::trace() << "NemoFeedbackWriter::write_variable_surf_qc: writing " << variable_name << std::endl;
  auto surf_var = ncFile->getVar(variable_name);
  surf_var.putVar(data.data());
...
void NemoFeedbackWriter::write_variable_surf_qc(const std::string & variable_name,
  ...
  oops::Log::trace() << "NemoFeedbackWriter::write_variable_surf_qc: writing flag_index: " << flag_index
                     << " of " << variable_name << std::endl;
  auto surf_var = ncFile->getVar(variable_name);
